...
The ChessCam repository provided multiple model formats, including PyTorch and ONNX. Before the models could be used, several preparatory steps were required. Netron was employed to inspect the ONNX files and determine the expected input and output. \\
...
The corner detection model identified the points where the edges of the inner squares on the chessboard met, commonly referred to as intersection points. Up to 49 intersection points could be detected in a single image. These points were illustrated in Figure \ref{fig:xcorners-chessboard}.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\linewidth]{figures/methods/ml-models/xcorners_chessboard.jpg}
    \caption[s]{Visualization of intersection points where the edges of the inner squares on the chessboard met. The model identified and predicted these points. Up to 49 intersection points could be detected in a single image.}
...
To track chess piece movements, it is necessary to identify the individual squares of the chessboard and associate each detected piece with its corresponding square. First the outer corners of the chessboard needs to be found. Given these corners, the board is divided into an 8x8 grid of equally sized squares. For each square, only the center point is considered for subsequent mapping.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\linewidth]{figures/methods/ml-models/outer_corners_centers_chessboard.jpg}
    \caption[S]{Visualization of the  outer corners of the chessboard  and the centers of each individual square, forming an 8x8 grid.}
    \label{fig:nms}
\end{figure}


Chess pieces are detected separately, and the bottom center of each piece’s bounding box is selected as its representative location. Each detected piece is then mapped to the nearest square center based on the minimum Euclidean distance. This mapping enables the determination of the position of each piece on the chessboard.

Let me know if you need it tailored further or want any section expanded!

When mapping a piece to 


To be able to find the squares the outer corners of the chessboard needs to be found. Once the squares are found we wish to assign each piece to their corresponding square. \\



By finding the corners one can calculate where the squares are located.
