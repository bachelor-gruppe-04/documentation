...
The corner detection model is responsible for identifying the chessboard corners and the squares. However, the corners-model doesn't directly find this. Instead it finds the intersection for the innersquares of the chess board, as illustrated in figure \ref{fig:xcorners-chessboard}


\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\linewidth]{figures/methods/ml-models/xcorners_chessboard.jpg}
    \caption[s]{Illustration of intersections}
...
It expects the same input as the piece-model \textbf{[1, 3, 288, 480]} (chatgpt write somethign about here about the above version having this and i dont want to repeat myself)

The corners-model outputs outputs data in the format \textbf{[1, 5, 2835]}. \textbf{1} represents the batch size. \textbf{2835} represents the number of anchor boxes used in the detection. \textbf{5} consists of two parts:
