...
Delen under må skrives litt bedre

Next, a perspective transformation is applied to map the corners of the board to a grid. This transformation begins by calculating the inverse of the matrix used to generate the corner locations during the previous steps. The inverse matrix allows to "undo" the original transformation, effectively reversing the process that mapped the real-world chessboard to its observed position. \\

Once the inverse matrix is computed, it is applied to the corner points, transforming them into a new set of coordinates. This step aligns the corners with a regular grid or a standard coordinate system, which represents the true position of the chessboard. As a result, the warped corner coordinates are obtained, and these can be used for further processing, such as rectifying the image, aligning the chessboard to a grid, or accurately estimating its position on the screen. \\

In essence, this perspective transformation corrects the perspective distortion caused by the observation angle, ensuring that the detected corners align correctly with the real-world layout of the chessboard. \\
