...
The ChessCam repository provided multiple model formats, including PyTorch and ONNX. The ONNX format was chosen because it is framework-agnostic, making integration into different deployment environments easier. It also simplifies the inference process. Before the models could be used, several preparatory steps were required. Netron, a model visualization tool, was employed to inspect the ONNX files and determine the expected input and output. This step was crucial in designing the appropriate preprocessing steps and correctly interpreting the model outputs. \\
