...
The team organized their work into bi-weekly \textbf{sprints}, each concluding with a status report and retrospective. \\

Status reports involved setting specific sprint goals, outlining the tasks each team member aimed to complete before the next meeting. These meetings also included a review of completed tasks, allowing the team to assess progress and discuss the outcomes of the previous sprint. If any tasks remained incomplete, the team identified potential causes and developed strategies to address them moving forward. During sprints, the product backlog was reviewed to select the next set of items to focus on, and action items were assigned to clarify responsibilities for each task. \\

Retrospectives provided a chance to reflect on the team's collaboration, discussing both strengths and weaknesses in teamwork. This allowed the team to identify challenges, recognize what worked well, and suggest improvements for future sprints, promoting continuous learning and better teamwork. \\

To manage and track tasks, the team utilized GitHub’s \textbf{Issue Board}. Issues were categorized based on its type: \textit{Enhancement} for new functionality or improvements, \textit{Documentation} for work related to adding explanations, improving code safety, or enhancing the understanding of code structure, \textit{Bug} for issues related to errors or unexpected behavior in the application, and \textit{Task} for general tasks not fitting into the other categories. Each issue was assigned to one or more team members to clarify responsibilities. The Issue Board was divided into four columns: \textit{No Status}, \textit{To Do}, \textit{In Progress}, and \textit{Done}, offering a clear visual overview of the development workflow. \\

To ensure code quality and encourage collaboration, the team practiced \textbf{code review} by using GitHub pull requests. New issues were implemented on separate branches, allowing team members to review each other’s contributions before merging them into the main codebase.
