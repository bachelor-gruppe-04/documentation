...
\begin{table}[h]
    \centering
    \caption[Offset values for anchor boxes - chess pieces]{Offset coordinates for each anchor box with regard to identifying chess pieces.}  % Caption moved to top
...
\begin{table}[h]
    \centering
    \caption[Offset values for anchor box - intersection points]{Offset coordinates for each anchor box with regard to identifying intersection points.}  % Caption moved to top
    \renewcommand{\arraystretch}{1.5} % Increase row height to allow text to be on top
    \begin{tabular}{lcccc}
        \toprule
        \textbf{Anchor box} & \textbf{xcenter} & \textbf{ycenter} & \textbf{width} & \textbf{height} \\
...
The team maintained regular office hours from 10:00 to 16:00 on weekdays, allowing for flexibility when needed. All working hours were documented in a shared log to ensure transparency and accountability. In addition to daily collaboration, the team consistently attended scheduled meetings with both the supervisor and the product owner, as well as other relevant activities related to the Bachelor project. \\

Most team \textbf{communication} occurred in person during office hours, supporting rapid feedback and close collaboration. For remote coordination, such as work outside of office hours or during individual development sessions, the team used Discord as the primary platform. Email was used for formal communication, such as scheduling meetings with the supervisor and product owner. \\

The team followed an \textbf{Agile-inspired methodology}, organizing work into biweekly \textbf{sprints}. Each sprint began with a planning session where the product backlog was reviewed, and priorities were set. Sprint goals were defined, and each team member committed to specific tasks to complete before the next sprint meeting. At the end of each sprint, a \textbf{sprint review} was held to evaluate progress and finalize a status report. Any incomplete tasks were discussed to identify challenges, and strategies were developed to address them in future sprints. \textbf{Retrospectives} provided space for reflection, helping the team identify what worked well and where improvements could be made.\\
...
All code contributions were made through \textbf{GitHub} using branches. Each issue was implemented in its own dedicated branch. Once completed, a pull request was created for peer review. The team have to review each other’s work, suggest improvements, and maintain high code quality before merging changes into the main branch. Descriptive and meaningful commit messages were emphasized throughout development. This helped maintain a clear and understandable commit history, which was especially valuable when revisiting earlier changes or troubleshooting issues.
