...
The ChessCam repository provided multiple model formats, including PyTorch and ONNX. The ONNX format was chosen because it is framework-agnostic, making integration into different deployment environments easier. It also simplifies the inference process. Before the models were used, several preparatory steps were required. Netron, a model visualization tool, was employed to inspect the ONNX files and determine the expected input and output. This step helped in designing appropriate preprocessing steps and correctly interpreting the model outputs. Additionally, due to limited documentation, clarifications regarding the model structure were obtained through direct communication with the original developers.
