...
To test the real-time communication via WebSocket, a lightweight simulator was implemented. This simulator sequentially sent hard-coded chess moves to connected clients, emulating live gameplay data. The frontend application was connected to the WebSocket endpoint during testing, enabling it to receive and display the incoming data. This setup allowed for effective verification of the WebSocket functionality and ensured that the frontend could handle and render live move data as intended.


\subsubsection*{REST API Testing with Postman}

To ensure the correct functionality of the \gls{rest}ful \gls{api} endpoints, Postman was used as a testing tool. Postman allows manual interaction with \gls{http} endpoints and is well-suited for verifying both request structure and server responses. Each of the implemented endpoints was tested individually, including actions such as resetting individual boards, resetting all boards simultaneously, retrieving the list of active boards, and requesting video streams from specific board cameras. \\

Figure~\ref{fig:postman-rest-tests} illustrates a sample of the \gls{api} calls tested through Postman, where various \texttt{POST} and \texttt{GET} requests were issued to endpoints such as \texttt{/reset/1}, \texttt{/video/1}, and \texttt{/boards}. These tests confirmed that the backend services responded as expected, including successful status codes, valid response payloads, and the intended effect on the system state. This form of manual \gls{rest} testing provided an effective means of validating \gls{api} correctness before integrating with the frontend interface.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.15\linewidth]{figures/results/postman.png}
