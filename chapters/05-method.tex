...
Each frame captured by the camera is processed by the two models. Before inference, the frame needs to match the models’ expected input dimensions [1, 3, 288, 480]. This process includes scaling the frame, normalizing the pixel values to fall within the range [0, 1] and converting the image to the appropriate data type. \\

After inference, the models produce the prediction outputs: [1, 16, 2835] for pieces and [1, 5, 2835] for intersection points. These outputs represent 2835 potential locations for both chess pieces and intersection points. To identify the most reliable predictions, low-confidence entries are first filtered out by applying a score threshold. Next, NMS is applied to further refine the results. This process ensures only the most relevant and accurate locations for the pieces and intersections. \\

To identify the board corners of the chessboard, Delaunay Triangulation is employed. The process aims at detecting potential quadrilateral shapes that may represent the four corners of the board. Delaunay triangulation is applied to the intersection points to generate a set of candidate triangles. This technique ensures that the intersection points are connected to form triangles, from which quadrilaterals can be identified. By iterating over the triangles and examining their shared vertices, the algorithm  identifies potential quadrilaterals that represent the four corners of the chessboard. \\

Once a list of potential quads is generated, each quad is scored which evaluates how well the quad fits with the original intersection points. The quad with the highest score is selected as the most likely representation of the board corners. \\

Next, a perspective transformation is applied to map the board corners onto a regular grid. This transformation begins by calculating the inverse of the matrix used to generate the corner locations during the previous steps. The inverse matrix allows us to "undo" the original transformation, effectively reversing the process that mapped the real-world chessboard to its observed position.

Once the inverse matrix is computed, it is applied to the predicted corner points, transforming them into a new set of coordinates. This step aligns the detected corners with a regular grid or a standard coordinate system, which represents the true position of the chessboard. As a result, the warped corner coordinates are obtained, and these can be used for further processing, such as rectifying the image, aligning the chessboard to a grid, or accurately estimating its position on the screen.

In essence, this perspective transformation corrects the perspective distortion caused by the camera or observation angle, ensuring that the detected corners align correctly with the real-world layout of the chessboard. By applying this transformation, the system can reliably interpret the board's position, which is crucial for tasks such as gameplay analysis or visual representation.
