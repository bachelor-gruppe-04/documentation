...
After conducting research into existing chess digitization methods, a publicly available solution called ChessCam was discovered \cite{lichess:chesscam}. This solution served as the foundation for the approach. It utilized two \glspl{cnn}: one for detecting chess pieces and another for locating the squares of the chessboard. \\

Although the documentation for the models was limited, the necessary logic and structure of the models were already present in the repository. When clarifications were needed, they were obtained through direct communication with the original developers, ensuring that the intended use and functionality of the models were correctly understood. \\

The ChessCam repository provided multiple model formats, including PyTorch and ONNX. Before the models could be used, it was necessary to identify their expected inputs and outputs. Netron was employed to inspect the ONNX files and determine these details. \\

\newpage


\subsection{Piece Detection Model}
The piece detection model was responsible for identifying and classifying chess pieces on the board. It expected \textbf{input} in the format \textbf{[1, 3, 288, 480]}. \textbf{1} represented the batch size, i.e. the number of images processed at once, which was typically set to \textbf{1} during inference. \textbf{3} corresponded to the number of color channels (RGB), indicating that the image had to be in color. \textbf{288} and \textbf{480} denoted the height and width of the image in pixels, respectively. \\
...
\begin{table}[h]
    \centering
    \caption{Example offset coordinates for each anchor box with regard to identifying chess pieces.}  % Caption moved to top
...
\begin{table}[h]
    \centering
    \caption{Example predicted class probabilities for each anchor box after applying the offsets.}  % Caption moved to top
    \renewcommand{\arraystretch}{1.5} % Increase row height to allow text to be on top
    \begin{tabular}{lcccc}
        \toprule
...
\begin{table}[h]
    \centering
    \caption{Example offset coordinates for each anchor box with regard to identifying intersection points.}  % Caption moved to top
    \renewcommand{\arraystretch}{1.5} % Increase row height to allow text to be on top
    \begin{tabular}{lcccc}
        \toprule
        \textbf{Anchor box} & \textbf{xcenter} & \textbf{ycenter} & \textbf{width} & \textbf{height} \\
...
\begin{table}[h]
    \centering
    \caption{Example predicted intersection probabilities for each anchor box after applying the offsets.}
...
Once the inverse matrix is applied, the resulting coordinates provide the corrected positions of the corners in the distorted image. This transformation compensates for the camera angle, ensuring the chessboard’s layout is restored to its true perspective. \\

Finally, labels were to be assigned to the corners, specifically: ‘a1’, ‘h1’, ‘a8’, and ‘h8’. To determine the optimal alignment, the average center of all black and white pieces was computed, and the board underwent four 90-degree rotations. For each rotation, the midpoints of the white and black sides (between ‘a1’ and ‘h1’, and between ‘a8’ and ‘h8’) were calculated. The rotation that minimized the distance between the piece centers and their corresponding midpoints was selected, indicating the best orientation. After identifying the optimal rotation, the labels ‘a1’, ‘h1’, ‘a8’, and ‘h8’ were applied to the corners accordingly. See Figure \ref{fig:board_label_assignment} for an illustration.
...
After detecting the chessboard and pieces, the application continuously processed each incoming video frame to detect potential moves. For each frame, the board state was updated based on the observed positions of the pieces. Each square maintained a set of 12 probabilities, one for each piece type, with the highest probability determining the piece present on that square. \\

To ensure stability, a decay-based mechanism was used to blend the previous state with new detections. This approach smoothed out transient errors caused by occlusions, noise, or inconsistencies, and prevented abrupt changes due to temporary issues. Updates were throttled at fixed intervals to balance performance and accuracy, avoiding unnecessary processing during minor fluctuations. \\

A move was only considered if it exceeded a predefined confidence threshold. Additionally, only legal moves were considered. If a high-confidence, legal move was detected, it was immediately executed to the board state. Once a move was confirmed, it was transmitted to the frontend via WebSocket, and the board state was updated accordingly for further analysis and visualization.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\linewidth]{figures/methods/ml-models/piece-model.png}
    \caption[Final piece detection result]{Final piece detection result showing the detected chess pieces along with their confidence scores. Each piece is mapped to its corresponding square on the board, enabling move detection and game state updates.}
