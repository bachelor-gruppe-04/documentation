...
\begin{figure}[h!]
    ...
    \caption[s]{Visualization of intersection points where the edges of the inner squares on the chessboard met. The model identified and predicted these points. Up to 49 intersection points could be detected in a single image. \cite{vectorstock:chessboard-svg}}
...
\begin{figure}[h!]
    ...
    \caption[S]{Example of a detected chess piece with its classification confidence. The bottom center of the bounding box is used as the reference point for mapping the piece to its corresponding square. \cite{svgrepo:black-pawn-svg}}
    \label{fig:nms}
\end{figure}
