...
\section{Detection Performance from ChessCam}

To establish a reference point for evaluating the performance of our system, we first consider the detection results reported in the original ChessCam repository~\cite{github:chesscam}. These metrics represent the upper-bound performance of the models.

The results presented in Figure~\ref{fig:chesscam-metrics} demonstrate that the piece model achieves consistently high detection performance across all key metrics under idealized conditions. Specifically, both precision and recall rapidly converge above 0.95. The mAP50 and mAP50-95 metrics also stabilize at high values, further confirming the model's strong overall detection accuracy. \cite{wandb:piece-detection}.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/results/machine-learning/chesscam-piece-detection.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/results/machine-learning/chesscam-xcorners-detection.png}
\caption{Performance metrics reported from the original ChessCam repository, based on YOLOv8 training.}
