...
The class diagram, shown in Figure~4.12, provides a static structural representation of the system’s object-oriented design. It outlines the key classes involved in managing the chess digitization process and illustrates their attributes, methods, and inter-class relationships. \\

At the top level, the App class serves as the main entry point for the desktop GUI application. It coordinates system setup and user interactions by instantiating core components such as BoardFactory, BoardService, and GUI elements like ProgressBarTopLevel and BoardResetSelectorTopLevel. These GUI components are responsible for managing camera setup, board resets, and visual feedback during live operation. \\

The BoardFactory class is responsible for creating and storing multiple Board objects, each representing a physical chessboard. Each Board instance aggregates a Camera object that interfaces with a connected webcam and a Detector instance that performs move detection using computer vision techniques. The Board class also maintains a move history and connected WebSocket clients, and exposes functionality for validating moves and resetting board state. \\

The BoardService class serves as the central controller for managing chess game flow. It interacts with Board instances to reset games, send moves to clients, and initiate asynchronous detector threads. The Detector class encapsulates the video processing logic, while CameraDoesNotExistError is a custom exception used to handle invalid hardware initialization. \\

Together, this diagram captures both the architectural hierarchy and the control flow pathways within the backend system. It supports an object-oriented implementation that cleanly separates concerns between data models, control logic, and user-facing components. \\
...
\begin{figure}[h!]
    ...
    \centering
    \begin{minipage}[t]{0.45\textwidth}
        The state machine diagram, shown in Figure~4.13, models the behavior of the move detection and validation subsystem within the chess digitization process. It depicts the distinct internal states the system transitions through when a player makes a physical move on the chessboard, and how the system responds accordingly. \\

        The process begins in the WaitingForMove state, where the system idles until a physical move is detected. Once a move is made, the system enters the DetectingMove composite state, which handles the image capture and processing pipeline. Inside this state, the DetectMove substate uses camera input to generate board imagery, and the MoveDetected substate represents the output generated by the machine learning model responsible for move recognition. \\

        Upon detecting a move, the system transitions to the ValidatingMove composite state. Here, the CheckMove substate verifies the legality of the detected move using chess rules. If the move is valid, the system enters the MoveValid state and transitions to DisplayMove, where the move is sent to the user interface for live display. The system then loops back to WaitingForMove to process the next move. If the move is invalid, the system transitions to the MoveInvalid state and terminates the process flow. \\

        This diagram highlights the system's reactive and event-driven nature, emphasizing how it transitions between detection, validation, and feedback states in response to player actions.
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.5\textwidth}
        \vspace{0pt}
...
The package diagram in Figure~4.14 presents the structural organization of the backend codebase. It follows a modular design under a primary logic package, which encapsulates the system's domain-specific functionality and separates concerns into smaller, well-defined subpackages. \\

Within logic, the machine_learning package contains submodules such as board_state, detection, maths, utilities, and view, each representing a specialized aspect of the ML pipeline used to analyze and interpret chessboard states. This modular structure facilitates easier testing and code reuse, particularly for board detection and move classification logic. \\

The api package manages routing and external interactions. It is divided into entity, routes, and services: \\

entity defines core data structures used in communication. \\

routes registers FastAPI endpoints. \\

services contains application logic that serves as the bridge between routes and the underlying model or system state. \\

Other components include resources, for static/shared files, and main.py, the central entry point that initializes the application and ties together the service and routing layers. This diagram highlights how the backend is organized around functional domains, enabling clear separation between system logic, API interfaces, and resource management. \\
...
Figure~4.15 illustrates the package-level structure of the frontend React application. The system is organized into multiple directories under the src folder, following standard React project conventions. \\

Key subdirectories include: \\

components: reusable UI elements such as camera, chessboard, and pgn display components. \\

pages: complete views like boardView, gamePreview, and tournamentView, each mapping to different routes in the application. \\

hooks: custom React hooks used for shared logic across components. \\

layouts: structural UI wrappers that maintain consistency across views. \\

data: potential mock or config data used to populate or simulate content during development. \\

Additionally, the public directory contains index.html, the static base template, while files like App.tsx and index.css serve as the core application logic and styling entry points, respectively. \\

This package structure reinforces maintainability and reusability by distinguishing between logic layers, reusable components, routing views, and styling/configuration resources. It reflects a scalable frontend architecture tailored to support interactive, live-updating chess content.
