...
After training and deploying a model, it is essential to evaluate its performance. In object detection, this is typically done using the following metrics. \textbf{Precision} measures the proportion of predicted bounding boxes that are correct (i.e., they sufficiently overlap with ground truth boxes). \textbf{Recall} measures the proportion of actual objects that the model successfully detects:
\[
...
Since there is often a trade-off between precision and recall, these metrics are combined using a precision–recall curve. The area under this curve is known as the \textbf{\gls{ap}}. \gls{ap} is calculated for each class individually, and the average across all classes gives the \textbf{mean Average Precision (mAP)}. A higher mAP value indicates that the model achieves both high precision and high recall across all object classes and IoU thresholds, making it one of the most comprehensive metrics for object detection performance.
