...
Since there is a trade-off between precision and recall, they are commonly analyzed together using a precision–recall curve. The area under this curve is known as the \textbf{\gls{ap}}. This is calculated for each object category individually. The average across all categories gives the \textbf{mean Average Precision (mAP)}. A higher mAP indicates that the model is both accurate and comprehensive in its predictions.
