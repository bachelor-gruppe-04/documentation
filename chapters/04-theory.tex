...
A model trained to recognize geometric shapes uses a labeled dataset consisting of various shape images paired with their respective names. Through exposure to these examples, the model learns to associate specific visual features with particular shapes. When presented with a new shape, the model attempts to classify it based on its learned knowledge. \\
...
\begin{figure}[h!]
    ...
    \caption[Example of anchor boxes in object detection]{Example of anchor boxes in object detection, used as reference points to predict object locations
...
\begin{figure}[h!]
    ...
    \caption[Non-maximum suppression (NMS) before and after applying the algorithm]{Non-maximum suppression (NMS) before and after applying the algorithm, showing the removal of overlapping bounding boxes \cite{thepythoncode:nms}}
    \label{fig:nms}
\end{figure}
