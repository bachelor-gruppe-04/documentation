...
A \gls{rest} \gls{api} is a programming interface that adheres to architectural limitations such as stateless communication and cacheable data. It isn’t a standard or a protocol. While REST \glspl{api} can be used via a variety of communication protocols, they are most typically accessed over \glspl{http}. \\

To understand the \gls{rest}, first we need to understand some key, The person or program that uses the \gls{api} is known as a client. The client sends queries to the \gls{api} in order to get information or make changes to the application. Your web browser is a client; it communicates with \glspl{api} on many websites to obtain page content. Your browser receives the required information and displays it on your screen. Any item of information that the \gls{api} may supply to the client is referred to as a resource. The program that receives client requests uses a server, which holds the resources that the client desires. The server has an \gls{api} that allows it to interact with clients without providing them direct access to database content.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\linewidth]{figures/theory/rest.png}
