...
The model's task is to detect the chess pieces on the board and assign them to their corresponding squares. By defining the starting position using a FEN string, the system supports game play from any desired position, whether starting mid-game or using alternative game modes. Built-in move validation ensures that only legal moves are processed and applied, allowing the system to maintain accurate and rule-compliant game progression. \\
...
\section{API}
%Discuss the desicion of chosing Websocket vs HTTP
...
\subsection{Download \gls{pgn} file}
